...
fn main() {
    ...
    app.connect_activate(|app| {
        ...
        // Definir las teclas
        let t = gdk::keys::constants::T;
        let minus = gdk::keys::constants::minus;
        ...
        window.connect_key_release_event(move |_, event| {
            if let Some(key) = event.get_keyval() {
                if key == t && event.get_state().contains(ModifierType::CONTROL_MASK) {
                    ...
                    tab_content.pack_start(&webview, true, true, 0);
                    ...
                    webview.navigate_uri("https://www.google.com").unwrap();
                    window_clone.show_all();
                    return Inhibit(true);
                } else if key == minus && event.get_state().contains(ModifierType::CONTROL_MASK) {
